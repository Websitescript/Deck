const term = {
  ...
  init() {
    ...
    // Clicking anywhere on the page (outside the title bar) refocuses the
    // input — this page has nothing else to interact with.
    document.addEventListener("click", (e) => {
      if (!e.target.closest(".term-titlebar")) this.focusInput();
    });

    // Delegated click handling for the interactive bits a command can
    // print into the output — links, copy buttons, and the done/not-done
    // checkboxes on notes & reminders (from /list, /notes, /reminders).
    this.outputEl.addEventListener("click", (e) => {
      ...
      if (noteToggle) {
        ...
        if (note) {
          ...
          const line = noteToggle.closest(".note-line");
      ...
      if (remToggle) {
        ...
        if (r) {
          ...
          const line = remToggle.closest(".note-line");
          if (line) {
