...
1. Open `index.html` — you'll land on an empty dashboard since there's
   nothing in it yet. Click the `Terminal | New Tab` tab (or the `>_
...
"Terminal | New Tab" tab is actually open somewhere — it's a plain
...
separate page — click the `Terminal | New Tab` tab up in the title bar,
